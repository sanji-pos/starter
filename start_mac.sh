...
docker run --pull always --platform linux/amd64 -e HOST_PRIVATE_IP=$HOST_IP -p 3000:3000 -v ./volumes/pg:/home/nextjs/database -w /home/nextjs/database --name local-sanji --rm sanjidev/gateway:latest || docker run --platform linux/amd64 -e HOST_PRIVATE_IP=$HOST_IP -p 3000:3000 -v ./volumes/pg:/home/nextjs/database -w /home/nextjs/database --name local-sanji --rm sanjidev/gateway:latest
